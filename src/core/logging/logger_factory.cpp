...
void LoggerFactory::init(const std::string& log_path, const std::string& log_level, const std::string& app_env) {
    try {
        ...
        // Rotating file sink
        sinks.push_back(std::make_shared<spdlog::sinks::rotating_file_sink_mt>(log_path, 10 * 1024 * 1024, 5));

        app_logger_ = std::make_shared<spdlog::logger>("app", sinks.begin(), sinks.end());
        ...
        auto sec_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(sec_log_path, 10 * 1024 * 1024, 5);
...
std::shared_ptr<spdlog::logger> LoggerFactory::security() { return security_logger_; }
