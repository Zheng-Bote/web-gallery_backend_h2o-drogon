...
/**
 * @class ConfigLoader
 * @brief Singleton class to manage application configuration via environment
 * variables.
 */
class ConfigLoader {
public:
   ...
   * @param key The environment variable key.
   * @return std::expected<std::string, std::string> The value or an error if
   * missing.
   */
  static std::expected<std::string, std::string>
  get_required(std::string_view key);
