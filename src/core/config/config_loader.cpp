 ...
 * @brief Implementation of .env file parsing and environment variable
 * management
 * @version 0.1.0
 * @date 2026-02-24
 *
...
std::expected<void, std::string> ConfigLoader::load(std::string_view path) {
  ...
  while (std::getline(file, line)) {
    ...
    auto end =
        std::ranges::find_if(line | std::views::reverse, is_not_space).base();

    if (start == line.end())
      continue;
    std::string trimmed(start, end);

    if (auto delimiter_pos = trimmed.find('=');
...
std::string ConfigLoader::get(std::string_view key,
...
ConfigLoader::get_required(std::string_view key) {
  auto val = get(key);
  if (val.empty()) {
    return std::unexpected(
        std::format("Missing required configuration: {}", key));
  }
  return val;
}
