...
void PhotoController::get_photos(const drogon::HttpRequestPtr &req,
    ...
    if (!result) {
        auto resp = drogon::HttpResponse::newHttpJsonResponse(nlohmann::json{{"error", result.error()}});
...
void PhotoController::get_photo_detail(const drogon::HttpRequestPtr &/*req*/,
                                     std::function<void(const drogon::HttpResponsePtr &)> &&callback,
                                     std::string id) {
    infra::repositories::PostgresPhotoRepository repo;
    ...
    if (!result.value()) {
        auto resp = drogon::HttpResponse::newHttpStatusCodeResponse(drogon::k404NotFound);
        callback(resp);
        return;
    }
...
void PhotoController::ping(const drogon::HttpRequestPtr &/*req*/,
                          std::function<void(const drogon::HttpResponsePtr &)> &&callback) {
