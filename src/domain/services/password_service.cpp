...
std::expected<std::string, std::string> PasswordService::hash_password(std::string_view password) {
    ...
    if (res != ARGON2_OK) {
        return std::unexpected(std::format("Argon2 hashing failed: {}", argon2_error_message(res)));
    }

    return std::string(encoded);
}

bool PasswordService::verify_password(std::string_view password, std::string_view hash) {
