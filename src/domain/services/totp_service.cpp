...
static std::vector<uint8_t> base32_decode(std::string_view input) {
    ...
    for (char c : input) {
        ...
        if (val == std::string_view::npos) continue;
...
std::string TotpService::generate_secret() {
    ...
    for (int i = 0; i < 32; ++i) secret += chars[rd() % 32];
    return secret;
}

bool TotpService::validate_code(std::string_view secret, std::string_view code) {
    if (code.size() != 6) return false;

    auto key = base32_decode(secret);
    uint64_t counter = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) / 30;
    ...
    auto check = [&](uint64_t c) {
        ...
        HMAC(EVP_sha1(), key.data(), (int)key.size(), msg.data(), msg.size(), hash, &len);
...
std::string TotpService::get_provisioning_uri(std::string_view user, std::string_view secret, std::string_view issuer) {
    return std::format("otpauth://totp/{}?secret={}&issuer={}", user, secret, issuer);
}
